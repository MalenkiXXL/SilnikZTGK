...
//	////tworzy pusty Shader Program. Shader Program to ostateczny obiekt, kt�ry "zszywa" Vertex i Fragment shader w jedn� ca�o��
...
//	unsigned int EBO;
//
//	//generowanie unikalnych ID dla obiekt�w
...
//#include <glm/gtc/type_ptr.hpp>
//
//// M�wimy kompilatorowi, �eby w tym pliku utworzy� cia�o biblioteki stb_image
//#define STB_IMAGE_IMPLEMENTATION 
//#include "stb_image.h"
//
...
//
//// Czas klatki (deltaTime to r�nica czasu mi�dzy obecn� a poprzedni� klatk�)
//GLfloat deltaTime = 0.0f;
//GLfloat lastFrame = 0.0f;
//
...
// Informujemy stb_image, �eby odwraca� tekstury na osi Y (OpenGL oczekuje punktu 0.0 na dole, a obrazki maj� go na g�rze)
//stbi_set_flip_vertically_on_load(true);

// W��czamy testowanie g��boko�ci (�eby obiekty z ty�u nie rysowa�y si� na obiektach z przodu)
//glEnable(GL_DEPTH_TEST);

// 5. Budowanie shader�w i wczytanie modelu
//Shader ourShader("shader.vert", "shader.frag");

// Wczytujemy model. Upewnij si�, �e plik char.obj le�y w folderze Twojego projektu!
//Model ourModel("BR_Charizard.obj");

// Opcjonalnie: odkomentuj, by widzie� siatk� (tryb wireframe)
// glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);
...
//// 6. G��WNA P�TLA GRY
    ...
    //    // Czyszczenie ekranu i bufora g��boko�ci na podany kolor (ciemnoszary)
    ...
    //    // Ustawianie macierzy Modelu (Pozycja, Obr�t, Skala)
    //    glm::mat4 model = glm::mat4(1.0f);
    //    model = glm::translate(model, glm::vec3(0.0f, -1.0f, -3.0f)); // Przesuwamy model lekko w d� i w g��b ekranu
    //    model = glm::scale(model, glm::vec3(1.0f, 1.0f, 1.0f));       // Dostosuj t� skal�, je�li tw�j char.obj jest gigantyczny lub mikroskopijny
    //    ourShader.setMat4("model", model);

    //    // FAKTYCZNE RYSOWANIE MODELU
    //    ourModel.Draw(ourShader);

    //    // Zamiana bufor�w i przetwarzanie zdarze� systemu Windows
    //    myWindow.OnUpdate();
    //}


// Odczytywanie wci�ni�tych klawiszy i poruszanie kamer�
//void proccessInput(GLFWwindow* window)
//{
//    if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
//        glfwSetWindowShouldClose(window, true);
...
//}
//
//// Obliczanie przesuni�cia myszy i obracanie kamery
//void mouse_callback(GLFWwindow* window, double xposIn, double yposIn)
//{
//    float xpos = static_cast<float>(xposIn);
//    float ypos = static_cast<float>(yposIn);
...
//
//    float xoffset = xpos - lastX;
//    float yoffset = lastY - ypos; // odwr�cone, bo y ro�nie od g�ry do do�u na monitorze
//
//    lastX = xpos;
//    lastY = ypos;
//
...
//}
//
//// Przybli�anie (Zoom) k�kiem myszy
//void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
//{
//    camera.ProcessMouseScroll(static_cast<float>(yoffset));
//}
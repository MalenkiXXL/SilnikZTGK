...
void GameGuiLayer::DrawOrderTickets(float gameX, float gameY, float gameWidth, float gameHeight, float baseScale) {
    ...
    for (auto it = m_ActiveOrderTickets.begin(); it != m_ActiveOrderTickets.end(); ) {
        ...
        if (isPlaying) {
            // ZMIANA: Usuwamy karteczk� tylko, gdy klient faktycznie odchodzi ze sceny,
            // ALBO jest obs�u�ony ale jakim� cudem omin�� faz� LeavingReaction
    ...
    for (size_t i = 0; i < m_ActiveOrderTickets.size(); ++i) {
        ...
        if (ticketTex) {
            ...
            glm::vec4 ticketColor = { 1.0f, 1.0f, 1.0f, 1.0f }; // Domy�lny, bia�y
            if (custScript && custScript->State == CustomerState::LeavingReaction) {
                if (custScript->m_WasCorrect) ticketColor = { 0.4f, 1.0f, 0.4f, 1.0f }; // Zielonkawy odcie� (dobrze)
                else ticketColor = { 1.0f, 0.4f, 0.4f, 1.0f }; // Czerwonawy odcie� (�le)
            }
...
void GameGuiLayer::OnUpdate(Timestep ts)
{
    ...
    if (!m_LevelCompletedPanel.IsOpen())
    {
        ...
        if (GameManagerScript::s_IsTutorialMode && GameManagerScript::s_ShowTutorialDialog)
        {
            ...
            if (GameManagerScript::s_IsTutorialMode && TutorialManagerScript::s_AllowSkip) {
                ...
                // Pobieramy now� tekstur� (skipTutoButton.png)
                ...
                if (isHovered && Input::IsMouseButtonPressed(0)) {
                    ...
                    if (skipHoldProgress >= 1.0f) {
                        ...
                        //// Resetujemy kamer� na OBECNEJ scenie, zanim silnik skopiuje jej zepsuty stan!
                        //if (m_ActiveScene) {
    ...
    if (s_CloudTransState > 0) {
        Input::SetUICaptureMouse(true); // Blokujemy klikanie na czas przej�cia!
        glEnable(GL_BLEND);
        glDisable(GL_DEPTH_TEST);
        Renderer2D::BeginScene(uiProj);
        ...
        // Rysowanie p�czniej�cej chmurki na�o�onej na wszystko
        if (m_BookCloudIcon) {
            float t = s_CloudTransProgress;
            float easeScale = t * t * (3.0f - 2.0f * t); // Smoothstep dla p�ynnego ruchu

            // Ekstremalnie du�a skala, �eby na pewno przykry� ca�y ekran

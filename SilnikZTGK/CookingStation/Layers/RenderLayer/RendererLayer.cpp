...
// Potrzebujemy dost�pu do GUI i Renderera2D, �eby rysowa� tekst jako obiekt
...
void RendererLayer::OnUpdate(Timestep ts) {
    ...
    // POPRAWKA: Je�li wychodzimy wczesnym returnem, MUSIMY odpi�� FBO!
    if (!activeScene) {
    ...
    // POBIERAMY MAGAZYN KOMPONENT�W ANIMATORA
    ...
    // Tworzymy projekcj� ortograficzn�, kt�ra idealnie pokrywa nasz ekran
    ...
    // W��czamy test g��boko�ci z powrotem, aby modele 3D poprawnie si� nak�ada�y
    ...
    if (activeScene->GetCamera()) {
        ...
        // WA�NE: Upewniamy si�, �e macierze �wiata s� aktualne przed cullingiem
        ...
        // 2. USTAWIAMY WSP�LNE DANE DLA OBU SHADER�W
        ...
        if (meshStorage && transformStorage) {
            ...
            for (auto& [modelPtr, batchData] : instancedBatches) {
                batchData.first->use();
                batchData.first->setBool("u_Animated", false); // Bezwzgl�dnie wy��czamy u_Animated!

                Renderer::SubmitInstanced(batchData.first, modelPtr, batchData.second);
            }

            // 4. RYSOWANIE OBIEKT�W ANIMOWANYCH (Pojedynczo, by klatki si� nie nadpisywa�y)
            for (auto& animDraw : animatedDraws) {
                animDraw.shader->use();
                animDraw.shader->setBool("u_Animated", true); // W��czamy flag� dla shadera

                // Pobranie i wys�anie transformacji ko�ci
      ...
      // 1. Zabezpieczenia dla przezroczysto�ci (Alpha Blending)
        glEnable(GL_BLEND);
            ...
            for (auto& scriptComp : scriptStorage->dense)
            {
                for (auto& scriptEl : scriptComp.Scripts)
                {
                    if((scriptEl.Name == "ParticleEmitterScript" || scriptEl.Name == "SteamEmitterScript" || scriptEl.Name == "DustEmitterScript") && scriptEl.Instance)
                    {
                        ...
                        {
                            if (!particle.Active) continue; // Nie rysuj u�pionych!

                            // 3. Interpolacja (p�ynne przej�cia w trakcie �ycia cz�steczki)
        ...
        Renderer2D::EndScene();
        glDepthMask(GL_TRUE); // Koniecznie w��czamy zapis g��bi z powrotem dla kolejnej klatki!
    }

    if (m_TargetFBO) {
        // Je�li mamy cel u�redniania (czyli m_ViewportFBO do kt�rego rysuje GUI)
        if (m_ResolveFBO) {
            m_TargetFBO->ResolveTo(m_ResolveFBO);
        }

        // ZAWSZE odpinamy FBO po zako�czeniu pracy warstwy!
        m_TargetFBO->Unbind();
    }
}

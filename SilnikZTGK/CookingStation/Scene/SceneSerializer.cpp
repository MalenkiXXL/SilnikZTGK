...
// Wczytuje dane z pliku do pami�ci i buduje �wiat w silniku
bool SceneSerializer::Deserialize(const std::string& path) {
	...
	if (data.contains("settings")) {
		...
		if (settings.contains("clear_color")) {
			...
			// BEZPIECZNE pobieranie kolor�w - je�eli nie ma alfy (c[3]), dajemy 1.0f
			float r = c[0];
			float g = c[1];
			float b = c[2];
	...
	if (data.contains("entities")) {
		...
		for (auto& item : data["entities"]) {
			...
			// Zapami�tujemy, jakie ID dosta� ten obiekt w stosunku do tego w pliku
			if (item.contains("id")) {
				std::size_t oldId = item["id"].get<std::size_t>();
				oldToNew[oldId] = newEntity;
			}

			// Je�li encja mia�a w pliku rodzica, dodajemy j� do kolejki 
	...
	// Na ko�cu SceneSerializer::Deserialize(...)
